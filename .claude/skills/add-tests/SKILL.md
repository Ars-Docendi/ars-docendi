...
- Mirar coverage manualmente y priorizar módulos críticos primero (Designaciones > Aulas/Portal > Tareas, ajustar según contexto). _(La skill `/test-gap-monitor` está diferida hasta que haya más código que escanear — ver `docs/quality/tech-debt.md` TD-003.)_
...
2. Tests existentes siguen pasando (`dotnet test backend/ArsDocendi.slnx` completo).
...
Ver [open-pr.md](../../../docs/workflows/open-pr.md). Título sugerido: `test: cover BR-<modulo>-NNN` o `test: smoke for <modulo>`.
