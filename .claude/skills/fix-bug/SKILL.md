...
Algo está roto, mal, o regresado — NO una nueva capability. Para nuevas capabilities usar `/add-feature`.
...
| Scope                                            | Acción                                                                |
| ------------------------------------------------ | --------------------------------------------------------------------- |
| Un solo módulo, sin cambio de Contracts          | Continuar este workflow — sin spec ni change                          |
...
1. **Backend**: `dotnet test backend/ArsDocendi.slnx` — todos verdes, incluido el test nuevo. Test específico: `dotnet test --filter "FullyQualifiedName~<NombreTest>"`.
...
Ver [open-pr.md](../../../docs/workflows/open-pr.md).
