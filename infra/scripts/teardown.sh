...
rol="app_${ambiente//-/_}"
psql_admin --set=app_db_user="$rol" -c "
  SELECT set_config('arsdocendi.app_db_user', :'app_db_user', false);
  DO \$\$
  BEGIN
    EXECUTE format('DROP ROLE IF EXISTS %I', current_setting('arsdocendi.app_db_user'));
  END
  \$\$;"
