...
2. **Grafo de dependencias DAG**. Sin ciclos. El registro contra el que se chequea es [backend/manifiesto-de-aristas.json](backend/manifiesto-de-aristas.json): un test lo compara con los `ProjectReference` reales de `backend/src` en tres direcciones —arista sin fila, fila sin arista, proyecto sin clasificar— y verifica la aciclicidad sobre el código. Toda arista nueva lleva su fila en el MISMO PR que el `.csproj`; una arista que excede un invariante lleva además `invariante` y `ticket`. La prosa que explica el grafo vive en [docs/architecture/dependency-graph.md](docs/architecture/dependency-graph.md).
...
14. **Frontera de motor para consulta generada**. Un módulo puede consultar schemas ajenos sin pasar por `Contracts` únicamente si la frontera está sostenida por el motor de base de datos y es **falsable**: rol de PostgreSQL sin GRANT de mutación, GRANT enumerados columna por columna contra un manifiesto versionado, policies RLS que conjunten el permiso de dominio, y tests que fallen si cualquiera de esas condiciones se degrada. Hoy aplica a `Modules.Asistente` y solo a él.

> **Por qué el #14 existe.** El asistente no puede cumplir el invariante #1: pasar por `Contracts` significaría que el modelo genere llamadas a métodos en vez de SQL, que es otro sistema. En vez de argumentar que la regla «ya lo permitía» —una regla reinterpretada deja de restringir a nadie y deja a `/architecture-drift-check` sin nada contra qué evaluar— se enmienda explícitamente. El texto viene de la decisión D2 de `openspec/changes/asistente-fundaciones/design.md`.
>
> La frontera de motor sólo es más fuerte que la convención **si es falsable**. Un `GRANT` que nadie re-verifica se degrada en silencio el día que alguien apunte el módulo a la cadena del rol dueño, y no se rompe nada: el sistema sigue funcionando, sólo deja de estar contenido. El invariante escrito más los tests son lo que convierte la afirmación en un hecho comprobable.
>
> **Ratificado el 2026-09-08 por Franco Garcete**, cerrando la tarea 0.1 de `asistente-fundaciones`. Dejó de ser una enmienda propuesta: es una regla del proyecto, y `/architecture-drift-check` la evalúa como tal. Lo que la sostiene está enumerado arriba y verificado por `ManifiestoPrivilegiosTests`, `PrivilegiosLecturaTests`, `RlsAlcanceTests` y `ArquitecturaAsistenteTests`; si alguno de esos deja de fallar ante una degradación, el invariante deja de valer aunque el texto siga acá.
