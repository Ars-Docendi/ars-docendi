> **Alcance ampliado el 2026-08-18 por decisión del usuario.** La infraestructura
> y las 21 tareas de test pendientes se implementan dentro de este mismo change.
> Los tests de integración usan PostgreSQL 18 real mediante Testcontainers; no se
> sustituyen triggers, índices parciales, `NULLS NOT DISTINCT`, `EXCLUDE` ni las
> carreras de concurrencia por dobles en memoria.
...
- [x] 3.5 Test: reejecutar `--migrate` sobre una base ya migrada es idempotente y termina con exit 0 — verificado en PostgreSQL 18 sobre una base temporal: la primera y la segunda ejecución terminaron con exit 0
- [x] 3.8 Crear `backend/tests/ArsDocendi.IntegrationTests`, integrarlo a la solución y montar un fixture aislado con xUnit v3 + Testcontainers.PostgreSql sobre PostgreSQL 18
- [x] 3.9 Fijar `SSH.NET` 2026.0.0 para corregir la vulnerabilidad alta transitiva de Testcontainers (`GHSA-q939-rpr3-3284`); `dotnet list package --vulnerable --include-transitive` no reporta paquetes vulnerables
...
- [x] 4.3 Test: se persiste una persona sin cuenta y sin legajo (caso Alta)
- [x] 4.4 Test: el primer login vincula `users.persona_id` a la persona existente sin duplicarla
- [x] 4.5 Test: documento duplicado es rechazado por la base
- [x] 4.6 Implementar `IVinculadorPrimerLogin`: crea o actualiza la cuenta de Azure AD y la vincula por documento explícito con la persona canónica, sin inferir identidad desde el UPN
...
- [x] 5.6 Test: crear un rol nuevo con scope se persiste con `es_sistema = FALSE`
- [x] 5.7 Test: modificar `code` o `scope` de un rol de sistema es rechazado; editar su `nombre` es aceptado
- [x] 5.8 Test: borrar un rol de sistema es denegado
- [x] 5.9 Test: crear un rol sin scope o con scope inválido es rechazado
- [x] 5.10 Test: `enforce_role_scope` aplica igual a un rol creado por el operador que a uno de sistema
- [x] 5.11 Test: revocar y volver a otorgar la misma asignación de rol es aceptado
- [x] 5.12 Test: otorgar dos veces el mismo permiso al mismo rol es rechazado
...
- [x] 6.4 Test: activar un segundo período sin desactivar el primero es rechazado por la base
- [x] 6.5 Test: un cargo fuera del catálogo es rechazado en pedidos y designaciones
...
- [x] 7.5 Test: segundo pedido para el mismo docente y período es rechazado, incluso desde otra cátedra
- [x] 7.6 Test: dos escrituras concurrentes hacen fallar exactamente una por el índice único
- [x] 7.7 Test: tras un pedido `rechazado` se puede crear uno nuevo en el mismo período
...
- [x] 8.3 Test: una designación cerrada no bloquea la apertura de una nueva sobre la misma materia
- [x] 8.4 Test: abrir una segunda designación vigente sobre la misma materia es rechazado
- [x] 8.5 Test: `origen_pedido_id` distingue una designación del circuito de una carga administrativa
...
- [x] 9.11 Test: un fallo parcial en la materialización de un Cambio revierte el cierre de la designación anterior
- [x] 9.12 Test: un rol con `es_sistema = FALSE` no puede aceptar, rechazar ni devolver en ninguna etapa
- [x] 9.13 Test: ningún `Modules.*` escribe sobre `personas`, `roles`, `permisos` ni `rol_permisos`
- [x] 9.14 Cablear el `AuditDbConnectionInterceptor` en el contexto nuevo (`AddInterceptors` en el registro de `IdentityDbContext`), de modo que `changed_by` no quede NULL. La verificación en runtime del GUC se incorpora a la suite ampliada de este change
...
- [x] 10.7 Alinear el mock de `features/roles` al schema real (scope, `es_sistema`) y agregar la advertencia al crear un rol de que no participa del circuito de aprobación
- [x] 10.8 Alinear el mock de `features/docentes` al catálogo único de cargos
...
- [x] 11.8 `docs/business-rules/designaciones.md`: BR-001 con su implementación en base de datos; BR-018 atado a `personas.legajo` nullable. El mapping se completa con la suite backend incorporada a este change
...
- [x] 11.10 `docs/product/designs/`: design spec del bloqueo por duplicado entre cátedras (invariante #12)
...
- [x] 12.1 Correr `openspec validate --strict` sobre specs y changes — este change y las 13 specs pasan; la validación global conserva fallos preexistentes en `admin-docentes` y `revision-tabla-agrupada-y-filtros`
- [x] 12.2 Correr el build y la suite completa de backend y frontend — backend Release 0 warnings/errores y 22/22 tests de integración; frontend build + lint verdes y 191/191 tests con `--testTimeout=15000` (el default de 5 s agota dos flujos de integración)
- [x] 12.3 Verificar `--migrate` sobre una base limpia y sobre una ya migrada
- [x] 12.4 Correr `/architecture-drift-check` y confirmar que no reporta drift nuevo
- [x] 12.5 Correr `/evaluate` contra las specs de este change y actualizar `docs/quality/scorecard.md`
