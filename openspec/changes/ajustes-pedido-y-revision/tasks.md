...
- [ ] 16.13 Validar con el cliente en el navegador.
...
- [ ] 17.14 Pendiente de decisión del cliente: los `font-size` de 11px/10px/9px y los de 15/17/22px
      quedan fuera de la escala tipográfica (que va de 12px a 36px). Se dejaron como están.
- [ ] 17.15 `TablaMisPedidos` sigue siendo una grilla a mano: migrarla a `Table` para no dejar dos
      criterios conviviendo.
- [ ] 17.16 Validar con el cliente en el navegador.
...
- [ ] 21.3 Abierto: el badge de `en_lote` dice "En lote" (etiqueta por defecto de `EstadoPedidoBadge`).
...
- [ ] 24.9 Nota de datos: todas las semillas están en el período "1", así que cambiar de período deja
