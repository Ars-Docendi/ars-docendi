...
- **Los tests son 1,44:1 contra el código productivo del backend, y eso es cobertura, no relleno.** De las 16.465 líneas de `Asistente/`, 10.925 son código, 2.880 son blancos, 2.173 llaves solas, 1.468 comentarios `//` y **980 XML-doc** (no 2.660: esa cifra de una versión anterior de este informe estaba mal medida). Las sentencias reales son ~7.380, el 45%. Hay 725 `[Fact]/[Theory]` con un cuerpo de **14,3 LOC de promedio** —258 tests de ≤10 líneas, 351 de 11-20, sólo 15 de más de 30—, así que no hay margen para «compactar tests». **Es la densidad más alta del repo: 22 LOC por test, contra 42 de Designaciones y 37 de Portal.**
- **Comparado con el resto del repo, esta suite no es la excedida: es la única que existe.** Medido: Asistente 725 tests (370 sin base de datos), Designaciones 32, Portal 25, Identity 29, Aulas y Tareas **cero**. Designaciones, Portal e Identity no tienen **ni un solo test unitario puro**: sus casos deterministas levantan una base Postgres igual, porque la clase base la hereda. Y cuatro reglas de negocio registradas —BR-designaciones-008, 011, 015 y 017— **no tienen hoy ningún test que las verifique**: su único mapping apuntaba a `frontend/src/features/designaciones/api/maquinaEstados.test.ts`, borrado en `d195e4b`, mientras `docs/business-rules/designaciones.md:166` sigue declarando «✅ Completado». Eso incumple el invariante #11.
