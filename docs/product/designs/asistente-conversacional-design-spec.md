...
| 👎 «¿Qué falló? Opcional» | Panel `--color-bg-surface` con pastillas de elección múltiple (`aria-pressed` independiente por pastilla): «Datos incorrectos», «No entendió la pregunta», «Faltan datos», «Otro»; comentario libre (textarea, placeholder «Contanos qué esperabas ver…», hasta 500 caracteres con contador, pista «No incluyas datos personales.»); «Omitir» / «Enviar comentario». Después: «Gracias. Tu comentario ayuda a mejorar el asistente.»                                                                                          |
...
| Aviso de deshacer         | Apariencia del `Toast` de la librería (fondo `--color-bg-raised`, borde `--color-border-strong`, filete izquierdo `--color-accent`, texto `--color-text-primary`), al pie del rail: «Conversación archivada» / «Conversación restaurada» / «Conversación eliminada» / «Conversaciones eliminadas» + «Deshacer», **10 s**. Uno a la vez; sigue visible con el rail colapsado (se superpone al hilo).                                                                                                                           |
...
| `oklch(14% 0.005 75)` / `98%`                        | aviso de deshacer (fondo / texto)                    | `--color-bg-raised` / `--color-text-primary` (ver desvíos)                                               |
...
| `oklch(74% 0.09 165)`                                | «Deshacer»                                           | `--color-text-link` en semibold (ver desvíos)                                                            |
...
- El panel 👎 sigue al mock 1:1 (PO-changed 2026-09-26): pastillas de elección múltiple y el
  campo «Contanos qué esperabas ver…», acotado a 500 caracteres con contador y la pista «No
  incluyas datos personales.» — el texto libre junto a una fila anónima se acota en vez de
...
- El aviso de deshacer no usa el fondo oscuro del mock: toma la apariencia del `Toast` de
  `@ars-docendi/ui` para coincidir con el resto de las notificaciones del sistema, y «Deshacer» se
  muestra como enlace (`--color-text-link` en semibold).
