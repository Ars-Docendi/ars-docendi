...
- **Jerarquía de rutas**: no existe una pantalla índice en `/designaciones`; las superficies accesibles son sus rutas hijas. El breadcrumb **no** incluye un nivel "Designaciones": los grupos del sidebar no son pantallas (ver "Convención de encabezados").
- **Mis pedidos** (`mis-pedidos-simplificado`): `Breadcrumbs` + `PageHeader` (acción "Nuevo pedido", deshabilitada sin período abierto) + tabla nativa del design system. Cada encabezado de datos — **Docente**, **Legajo**, **Cátedra**, **Tipo**, **Enviado** y **Estado**— ofrece un filtro compacto; los textos buscan sin distinguir mayúsculas ni tildes, las opciones múltiples usan OR y las columnas distintas AND. Esos filtros se aplican antes del ordenamiento; la tabla no pagina (ver "Patrón transversal — grillas"). Los encabezados de datos también alternan orden ascendente, descendente y sin orden manual; **Acciones** no ofrece filtro ni orden. Cada fila: docente (sin prefijo "Prof."), **legajo** (o "—" si el docente todavía no tiene uno, p. ej. una Alta recién cargada), cátedra, **Tipo** (la novedad — antes "Novedad"), fecha de envío, `EstadoPedidoPill`. La fila entera es **clickeable** y navega al detalle del pedido. Al final de la fila: botón **"Ver"** (siempre visible, mismo destino que el click en la fila) y botón **"Editar"** (solo si la API incluye la acción `editar`) — ambos con el **mismo formato que las acciones de fila de Usuarios** (`Button variant="ghost" size="sm"`, sin ícono); y, únicamente cuando la API incluye `eliminar`, un botón de texto **"Eliminar"** (en rojo) que abre `ModalConfirmarEliminar` antes de borrar. Ninguna de estas acciones dispara la navegación al detalle (`stopPropagation`). Sin menú kebab, sin acciones de enviar/cancelar en la lista — "Enviar"/"Reenviar" se mudaron al formulario (ver más abajo) y "Cancelar" (pasar a `cancelado`) no tiene reemplazo en esta iteración (Eliminar cubre el caso de un borrador no deseado, ver Decisiones de diseño).
- **Períodos de designación**: `PageHeader` ("Períodos") + tabla nativa. Acciones de fila como en Mis pedidos: botón **"Editar"** (`Button variant="ghost" size="sm"`) + botón de texto **"Eliminar"** en rojo, que abre `ModalConfirmarEliminar` (sin menú kebab). El click en la fila abre Editar. Los campos de fecha del modal usan `DatePicker`, con un único ícono de calendario dentro del campo. **Nombre**, **Carga desde**, **Carga hasta**, **Impacto desde**, **Impacto hasta** y **Activo** ofrecen filtro por encabezado; Nombre y fechas buscan por texto normalizado y Activo permite seleccionar Activo/Inactivo, individualmente o ambos. Las mismas columnas alternan orden ascendente, descendente y sin orden manual; sin orden manual se conserva el orden predeterminado por Impacto desde descendente. **Acciones** queda sin filtro ni orden.
- **Formulario de pedido**: `Breadcrumbs` (Inicio › Mis pedidos › Nuevo pedido, o Inicio › Detalle del pedido › Editar pedido) + `PageHeader` (título "Nuevo pedido" / "Editar pedido", meta = subtítulo según novedad/edición) + **tarjeta** (máx. 860px) con secciones de eyebrow mono por novedad:
...
- **Eliminar con confirmación, sin justificativo** (`mis-pedidos-simplificado`): `ModalConfirmarEliminar`, el diálogo de borrado único de la app (título, texto con el nombre del docente, aviso "no se puede deshacer", Cancelar/Eliminar, estado de carga) — no el de `ModalConfirmacionAccion` (pensado para acciones de revisión con justificativo y aviso de a quién se notifica, que no aplica: eliminar un borrador propio no notifica a nadie).
...
- `Breadcrumbs` según el origen: `Inicio › Mis pedidos › Detalle del pedido` o `Inicio › Revisión › Detalle del pedido`. Mis pedidos y Revisión pasan el origen en el estado de navegación; con un link directo se deduce por permiso (`designaciones.revisar` → Revisión, si no → Mis pedidos), para no llevar nunca a una pantalla sin permiso. Un botón **Volver** persistente (vuelve con el historial). El propietario actual de un `devuelto` (Jefe, Coordinador o Secretaría), y el Jefe propietario de un `borrador`, ven **Editar** cuando la API incluye esa acción. Sólo el borrador ofrece además **Eliminar**. Sigue el `PageHeader`: título = tipo de novedad (p. ej. "Cambio de cargo o dedicación"), meta = período, y `EstadoPedidoBadge` a la derecha. El encabezado no repite número de pedido, cátedra ni carrera: ya están en la tarjeta "Datos del pedido".
...
## Patrón transversal — encabezados de página

Todas las pantallas autenticadas usan la misma estructura (change `ajustes-ui-varios`):

- **Breadcrumb**: `Inicio › Página`, o `Inicio › Pantalla de origen › Página` para pantallas hijas (detalle, formulario). Nunca incluye el grupo del sidebar (Personal, Trabajo, Designaciones, Configuración): no es una pantalla y quedaría como un nivel no clickeable.
- **Título**: igual al label del sidebar, con mayúscula solo en la primera palabra ("Revisión", "Períodos", "Reserva de aulas"). Excepciones: "Administración de usuarios / docentes / roles", y "Mis docentes" para el Jefe de Cátedra.
- **Sin pretitle**: la línea mono en mayúsculas sobre el título se eliminó; la sección ya la indica el breadcrumb.
- **Meta**: solo datos que no aparecen en otro lugar de la pantalla (conteos, período). Nunca el rol, que ya está en la barra superior.

## Patrón transversal — acciones por fila

Las tablas muestran sus acciones como **botones de texto** en la columna Acciones, sin menú kebab ni íconos sueltos: `Button variant="ghost" size="sm"` para "Ver", "Editar", "Desactivar", y "Eliminar" en rojo (`shared/ui/BotonEliminarFila`). Así lo hacen Mis pedidos, Períodos, Usuarios y Docentes; Revisión no tiene columna Acciones. Los usuarios son personal administrativo con poca experiencia tecnológica: nunca una acción solo con ícono ("iconos sin label" es anti-pattern en `design-principles.md`).

- **Click en la fila = acción principal**: abre el detalle si la grilla lo tiene (Mis pedidos, Revisión), **sin botón "Ver"**; si no, abre Editar en su modal (Períodos, Usuarios, Docentes). Solo cuando el usuario puede ejecutar esa acción; si no, la fila no es clickeable. La fila se enfoca con Tab y se abre con Enter o Espacio. Los botones y links de la fila hacen lo suyo y seleccionar texto no dispara nada (`shared/ui/filaClickeable`).
- **Sin columnas fijas**: se probó fijar Acciones a la derecha y se descartó (tapaba datos y quedaba vacía en muchas filas).

- **Solo acciones reales y habilitadas** (invariante #7): se muestran únicamente las acciones que el estado y el rol permiten.
- **Borrado siempre confirmado**: toda eliminación abre `ModalConfirmarEliminar` (`shared/ui`), el diálogo único con qué se borra, "Esta acción no se puede deshacer.", estado de carga mientras borra y el error dentro del diálogo.
- El menú kebab ⋮ (`shared/ui/MenuAcciones`) queda solo en las listas del Portal docente, que no son tablas.
- Los frames `ICs06` (Períodos) y `A9tdD` (Administración de Usuarios) de `screens.pen` todavía muestran el menú kebab: quedan como deuda de mockup.

## Patrón transversal — grillas

Las cinco grillas (Mis pedidos, Revisión, Períodos, Usuarios y Docentes) comparten el mismo comportamiento (change `grillas-unificadas`):
