...
- **Formulario de pedido**: `Breadcrumbs` + encabezado propio (eyebrow `DESIGNACIONES · <NOVEDAD>` + título + subtítulo según novedad/edición) + **tarjeta** (máx. 860px) con secciones de eyebrow mono por novedad:
  ...
  - Botonera (alineada a la derecha, con separador): Cancelar (secundario, descarta la edición sin guardar) + **Guardar pedido** (secundario, guarda sin enviar) + **Guardar y enviar** / **Guardar y reenviar** si el pedido está `devuelto` (primario, guarda y envía/reenvía en el mismo paso — `mis-pedidos-simplificado`), con la misma validación que "Guardar pedido".
...
- **Validación inline bloqueante**: el submit inválido no se envía; el error aparece en el campo (`Field error`) o como `InlineAlert` (adjuntos). Las reglas mapean a BR-001..004, BR-018.
