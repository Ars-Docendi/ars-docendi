...
- **Formulario de pedido**: `Breadcrumbs` + encabezado propio (eyebrow `DESIGNACIONES · <NOVEDAD>` + título + subtítulo según novedad/edición) + **tarjeta** (máx. 860px) con secciones de eyebrow mono por novedad:
  ...
  - **Datos del docente**: en Alta son inputs nuevos (DNI + Apellido y Nombre); en Sin novedad / Baja / Cambio es un `Select` de docente existente + panel de **datos actuales** read-only (Antigüedad · Cargo actual · Dedicación actual · Materia).
  - **Designación solicitada** (Alta/Cambio): materia (`Select`, solo Alta) + cargo + dedicación solicitados.
  - **Justificación** (Alta/Baja/Cambio): "Motivo del pedido" / "Motivo de la baja" (`Textarea`) + `Toggle` "hace más horas en otro Departamento" (Alta/Cambio).
