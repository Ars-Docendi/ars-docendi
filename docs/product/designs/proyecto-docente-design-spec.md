...
- **Formulario de pedido**: `Breadcrumbs` + encabezado propio (eyebrow `DESIGNACIONES · <NOVEDAD>` + título + subtítulo según novedad/edición) + **tarjeta** (máx. 860px) con secciones de eyebrow mono por novedad:
  ...
  - **Datos del docente**: en Alta son inputs nuevos (DNI + Apellido y Nombre); en Sin novedad / Baja / Cambio es un `Select` de docente existente + panel de **datos actuales** read-only (Antigüedad · Cargo actual · Dedicación actual). **En Cambio, el panel se convierte en un resumen de cambios**: cada campo que difiere de lo solicitado se muestra como transición `actual → solicitado` (viejo en gris tenue, flecha, nuevo en negrita/verde acento — mismo lenguaje visual para cargo, dedicación, materia con sus horas, y horas de investigación/externas; sin cambios se ve el valor plano).
  - **Designación solicitada** (Alta/Cambio): cargo solicitado (`Select` libre entre todo el catálogo, sin restricción de jerarquía) + dedicación solicitada (**en Cambio, el `Select` solo ofrece dedicaciones jerárquicamente mejores que la actual** — la escala es descendente, Categoría 0 es la de mayor jerarquía y Categoría 6 la de menor; en Alta no hay restricción) + la materia del pedido en solo lectura con sus horas editables + horas de investigación / horas externas (otro depto., campos numéricos libres, sin cierre contra la dedicación).
  - **Materia del docente** (Baja): la materia y las horas del pedido, íntegramente de solo lectura — es contexto de qué queda vacante.
...
- **Docente según novedad**: Alta carga un docente nuevo (inputs DNI + Apellido y Nombre); Sin novedad / Baja / Cambio seleccionan del catálogo servido por la API y muestran su designación vigente en solo lectura.
...
- Modelo de agrupación (`revision-tabla-agrupada-y-filtros`, quinta ronda): la tabla se agrupa en **4 secciones desplegables por etapa del circuito** — **En Coordinación · En Secretaría · En Decanato · Finalizados** (Aceptados + Rechazados juntos, sin sub-secciones) —, no por estado de avance como se había planeado originalmente (opción D). Un pedido **Devuelto** no tiene sección propia: vive en la sección de la etapa a la que volvió (`etapaRetorno`), con su celda Estado mostrando el **mismo mini-stepper + "En {etapa} · {paso}/{total}"** que un estado en revisión activa (calculado sobre `etapaRetorno`, no sobre `estado`), con **" · Devuelto por {revisor} ({rol})"** agregado al final de esa misma línea (decimoséptima ronda: el stepper y la etapa no se reemplazan por un punto de color — sacarlos por completo eliminaba el dato que un revisor necesita para el filtro "Mis pendientes"). Motivo del cliente: Secretaría Académica, Administrativo y Decanato ven **todo el departamento** (a diferencia del Coordinador, que ve solo su carrera — BR-009) y necesitan triangular grandes volúmenes de pedidos por dónde están trabados en la cadena, no por si ya se cerraron.
...
- **Expansión por default según el rol del actor** (`revision-tabla-agrupada-y-filtros`, sexta ronda — reemplaza "las 4 arrancan expandidas" de la quinta): arranca expandida **solo** la sección del rol del actor — Coordinador → "En Coordinación", Secretaría → "En Secretaría", Decanato → "En Decanato" —; las demás arrancan colapsadas. Administración no tiene sección propia → las 4 arrancan colapsadas para ese rol (instrucción explícita del cliente: "da igual" cuál). Cada sección expandida muestra su **propio head de columnas** (ya no uno solo compartido arriba de las 4).
- **Separación visual entre secciones** (sexta ronda): cada una de las 4 secciones es su propia card (borde + esquinas redondeadas + fondo), separadas entre sí por espacio — ya no viven pegadas dentro de un único contenedor. El título de cada sección ("En Coordinación", etc.) usa un color de acento, distinto al gris de los headers de columna, para no confundirlos.
- **Card en `--neutral-100` + franja de color en el borde izquierdo** (octava→duodécima ronda): el fondo del header pasó de `--color-bg-canvas` (mismo token que el fondo de página — se perdía contra el fondo) a blanco puro (`--color-bg-raised` = `#ffffff`), a dos tonos de verde de fill completo (`--success-100`, luego `--accent-200`), y de vuelta a blanco — cada uno rechazado por el cliente ("está horrible" al verde; "no combina con el fondo beige" al blanco puro). Diagnóstico: el fondo de página (`--color-bg-canvas` = `--neutral-200`) es un cálido casi-neutro (matiz 75°, croma ~0.006). El verde de marca vive en matiz ~150-165° — choca por matiz opuesto. `--color-bg-raised` es `#ffffff` literal, sin matiz — choca por ausencia de matiz junto a un cálido. Solución final: `--neutral-100` (98% de luminosidad, mismo matiz 75° que el fondo, apenas más claro que `--neutral-200`) + `border-left` de 4px en `--accent-500` (identidad de color sin teñir toda la superficie) + sombra sutil.
- **Contador de sección** (séptima→duodécima ronda): el badge del header de cada sección muestra **"Total: {n}"** (no solo el número); pill de color (`--accent-100`/`--accent-700`) — con la card en un neutro claro, un área chica de acento no genera el choque que sí tenía cuando toda la card estaba teñida de verde.
- **Fondo del head de columnas = fondo del título** (decimocuarta ronda, revierte un intento intermedio en `--neutral-300` que el cliente rechazó): dentro de cada sección expandida, el head de columnas (Docente/Legajo/Asignatura/Tipo/Fecha/Estado) usa el mismo `--neutral-100` que el título del desplegable — separados por una línea divisoria (`border-top`, decimoquinta ronda: al compartir color quedaban fundidos en un solo bloque sin nada entre medio), no por un cambio de color.
- `Breadcrumbs` + `PageHeader` ("Tablero de revisión de pedidos", subtítulo "Pedidos en tu ámbito · {rol} · {ámbito}", acción **Vista** — "Mis pendientes"/"Vista completa", sin switcher de Kanban/Tabla) + **filtro estilo Mis Pedidos** — mismo componente genérico `shared/ui/FiltrosLista.tsx`: **Nombre** del docente y **Tipo** de novedad como campos siempre visibles al inicio de la barra (sexta ronda: Tipo pasa de opcional a fijo, junto a Nombre — rompe a propósito la paridad de patrón con Mis Pedidos que se había buscado en la segunda ronda), más **Legajo** y **Prioridad** como filtros opcionales vía "+ Añadir filtro".
- **Columnas de la tabla** (`revision-tabla-agrupada-y-filtros`, tercera y cuarta ronda): `Docente` (avatar de iniciales + nombre, **sin prefijo** — cuarta ronda sacó el "Prof." que tenía antes) · `Legajo` (legajo del docente, "—" si todavía no tiene — caso de una Alta) · `Asignatura` (filler) · `Tipo` (chip Alta / Baja / Cambio — mismo chip que antes se tituló "Novedad", header renombrado a "Tipo" para paridad textual con Mis Pedidos) · `Fecha última actualización` (fecha del evento más reciente del historial del pedido, cualquier acción, formato dd/mm/aaaa) · **`Estado`** (columna **combinada** estado + avance, en una línea: en revisión → mini-stepper accent + "En {etapa} · x/4"; aceptado → punto de color + "Aceptado" (sexta ronda: ya no el stepper completo de 4 barras, mismo lenguaje visual que devuelto/rechazado — es un estado terminal, sin avance que mostrar); devuelto → mismo mini-stepper accent + "En {etapa} · {paso}/{total}", calculado sobre `etapaRetorno`, más " · Devuelto por {revisor} ({rol})" al final de la línea (decimoséptima ronda; sin `etapaRetorno` —no debería pasar— cae a punto de color + solo "Devuelto por {revisor} ({rol})"); rechazado → punto de color + "Rechazado") · `Prioritario` (columna **solo-ícono**: bandera roja si es prioritario, vacía en el resto).
...
- **Implementación (frontend real)**: la vista Tabla (`TablaRevision` + `SeccionEstadoTabla` + `EstadoAvance`) reusa el modelo de agrupación (`construirColumnas`, `avancePedido`, `quienDevolvio`, …, en `tableroRevisionModelo.ts`). `construirColumnas` ya no toma el actor — el orden dentro de una sección no depende de "tu turno" (con secciones ya particionadas por etapa, ese eje perdía fuerza: ver D-6 de `design.md`). El default del filtro de turno se mantiene en **"Vista completa"**: se evaluó "Mis pendientes" pero deja la tabla casi vacía, porque los estados terminales no son "tu turno". El **motivo de un pedido rechazado** ya no se muestra en la tabla (no hay card); se mudó al detalle (`ResumenPedido`, ver más abajo). Changes OpenSpec: `tablero-revision-vista-tabla`, `rediseno-revision-solo-grilla`, `revision-tabla-agrupada-y-filtros`.
