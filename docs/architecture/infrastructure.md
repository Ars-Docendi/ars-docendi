...
```
   ...
   │                              │                                   │
   │                      ┌───────────────┐                          │
   │                      │ SeaweedFS + ClamAV│ red interna solamente   │
...
- **SeaweedFS y ClamAV**: sólo alcanzables por `arsdocendi-datos`; SeaweedFS no tiene labels de Traefik, puertos publicados ni consola pública. La aplicación usa credenciales por ambiente y un bucket aislado.
