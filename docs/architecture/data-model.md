...
| `asistente.retroalimentacion_turno` | `analitico_id` (FK a `registro_analitico.id`, `ON DELETE CASCADE`), `voto`, `razon` (`CHECK` con 5 valores: los 4 vigentes más el legacy `lento`), `actualizado_en`             | El actor, y cualquier columna que exista en `registro_operativo` |
...
**El `CHECK` de `razon` se ensanchó en el lugar, no se recreó desde cero (asistente-rediseno-v3, design.md D7).** El conjunto vigente de la API pasó de `{datos_incorrectos, no_entendio_la_pregunta, lento, otro}` a `{datos_incorrectos, no_entendio_la_pregunta, faltan_datos, otro}`: `lento` se retira de la API (`RazonesDeRetroalimentacion.Todas` ya no lo lista; una votación nueva con `lento` da `400`) pero sigue **legal en la columna**, porque el `CHECK` no se puede angostar sin invalidar filas ya escritas. `003_asistente_retroalimentacion.sql` declara el `CHECK` con los 5 valores en el `CREATE`, y agrega un bloque `DO` que lo reemplaza (`DROP CONSTRAINT IF EXISTS` + `ADD CONSTRAINT ... CHECK`) **solo** cuando la definición vigente todavía no incluye `faltan_datos` — así una base provisionada antes de este cambio converge sin rechazar ningún voto «Faltan datos», y una base ya al día no vuelve a tocar el catálogo. Es la **única excepción ratificada** a la prohibición de `DROP` de `ArquitecturaAsistenteTests` (`ReemplazosDeCheckRatificados`, por nombre de constraint): un `CHECK` sólo se puede ensanchar destruyéndolo y recreándolo, nunca con un `ALTER ... ADD` incremental. Las filas con `razon = 'lento'` de antes de este cambio quedan intactas y envejecen con la purga de 90 días de `registro_analitico` (TD-012), sin reescribirse a `otro` — sería falsear un feedback ya registrado.
...
**`turno_historico.referencias`** (jsonb, nullable) queda reservada para `asistente-menciones` (design.md D11): marcador `$refN` → tipo e id de la mención citada en la SQL de ese turno. Nula en todo turno sin menciones — el caso normal hoy, porque nada del módulo la escribe todavía.
